...
| [`agent-audit`](presets/agent-audit.toml) | AI-agent workflow reproducibility | run manifests ("lockfile"), config drift diff, tool-call trace alignment (call_id-paired, exact/inferred quality, first behavioral divergence), multidimensional outcome fingerprint, two-stage classed decision extraction (markers, then optional @semantic slot) with evidence spans, recursive secret redaction (public output + blame trail) |
