...
from throughline.errors import RegistryError
from throughline.registry import RegistryEntry, check_kind
...
class KindChecksTests(unittest.TestCase):
    ...
    def test_middleware_check(self):
        self.assertIsNone(check_kind(tl.modules.Retry(), "middleware"))
    ...
    def test_store_check_accepts_both_shapes(self):
        ...
        self.assertIsNone(check_kind(tl.MemoryArtifactStore(), "store"))  # put/get
    ...
    def test_store_subkinds_pin_the_protocol(self):
        ...
        cache, artifact = DummyStore(), tl.MemoryArtifactStore()
...
class TypedRegistryTests(unittest.TestCase):
    ...
    def test_register_default_kind_is_step(self):
        ...
        def clean(text):
            return text
        self.assertIs(tl.resolve("clean"), clean)
        self.assertIs(tl.resolve("clean", kind="step"), clean)

    def test_register_typed(self):
        store = DummyStore()
        tl.register("mystore", store, kind="store")
        self.assertIs(tl.resolve("mystore", kind="store"), store)

    def test_kind_mismatch_is_loud(self):
        tl.register("mystore", DummyStore(), kind="store")
        with self.assertRaises(RegistryError) as caught:
            tl.resolve("mystore", kind="step")
        self.assertIn("registered as store, not 'step'", str(caught.exception))

    def test_same_name_multiple_kinds_prefers_slot_kind(self):
        tl.register("thing", lambda p: p, kind="step")
        store = DummyStore()
        tl.register("thing", store, kind="store")
        self.assertIs(tl.resolve("thing", kind="store"), store)
    ...
    def test_precedence_local_beats_plugin(self):
        ...
        self.assertEqual(tl.resolve("x"), "from-local")
    ...
    def test_manifest_registration(self):
        ...
        self.assertIsNotNone(tl.resolve("redis", kind="store"))
        self.assertIsNotNone(tl.resolve("plainname", kind="step"))

    def test_requires_gate(self):
        self.assertIsNone(registry._compatible("throughline>=0.1"))
        self.assertIn("requires throughline>=99", registry._compatible("throughline>=99.0"))
        self.assertIn("unsupported requires spec",
                      registry._compatible("throughline==1.0"))

    def test_entries_catalog(self):
        tl.register("a", lambda p: p, kind="step")
        tl.register("b", DummyStore(), kind="store")
...
class CustomKindsTests(unittest.TestCase):
    ...
    def test_namespaced_kind_registers_and_resolves(self):
        ...
        tl.register("fast", reranker, kind="acme.reranker")
        self.assertIs(tl.resolve("fast", kind="acme.reranker"), reranker)
        self.assertEqual(registry.entries()[0].kind, "acme.reranker")

    def test_bare_unknown_kind_is_rejected_at_registration(self):
        with self.assertRaises(RegistryError) as caught:
            tl.register("x", lambda p: p, kind="reranker")
    ...
    def test_declared_protocol_is_enforced(self):
        tl.register_kind("acme.reranker",
    ...
    def test_register_kind_rejects_builtin_and_bare_names(self):
        with self.assertRaises(RegistryError):
            tl.register_kind("step", check=callable)
        with self.assertRaises(RegistryError):
            tl.register_kind("reranker", check=callable)
    ...
    def test_manifest_with_namespaced_kind(self):
        ...
        self.assertIsNotNone(tl.resolve("fast", kind="acme.reranker"))

    def test_kind_mismatch_message_covers_custom_kinds(self):
        tl.register("fast", lambda d: d, kind="acme.reranker")
        with self.assertRaises(RegistryError) as caught:
            tl.resolve("fast", kind="step")
    ...
    def test_builtin_slot_wins_name_collisions_over_custom(self):
        tl.register("thing", "custom-kind-obj", kind="acme.widget")
        tl.register("thing", "step-obj", kind="step")
        self.assertEqual(tl.resolve("thing"), "step-obj")
